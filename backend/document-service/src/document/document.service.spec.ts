...
import { DocumentDto } from './document.schema';

describe('DocumentService', () => {
  let service: DocumentService;
  let postModel: Model<DocumentDto>;
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        DocumentService,
        { provide: getModelToken(DocumentDto.name), useValue: mockPostModel },
        { provide: CACHE_MANAGER, useValue: mockCacheManager },
      ],
    }).compile();

    service = module.get<DocumentService>(DocumentService);
    postModel = module.get<Model<DocumentDto>>(getModelToken(DocumentDto.name));
